...
/*
void PositivityConstraint::Compute(FlagType r){
    Clean(r);
    const size_t n = val.size();
    if(r & RLogGrad) logGrad.reserve(n);
    if(r & RLogHessian) logHessian.reserve(n);
    if(r & RValues) values.reserve(n);
    if(r & RJacobian) jacobian.reserve(n);
    
    IndexType counter=0;
    for(auto x : val){
        if(r & RLogSum) logSum -= log(x);
        if(r & RLogGrad) logGrad.push_back(-1./x);
        if(r & RLogHessian) logHessian.push_back({counter,counter,1./(x*x)});
        if(r & RValues) values.push_back(x);
        if(r & RJacobian) jacobian.push_back({counter,counter,1});
        ++counter;
    }
