...
void Monopolist_NoBound(PointSet pts, std::string filename){
	...
	// In this approach, we add a few points, which help handle the boundary conditions.
	// Find a bounding box
	std::array<ScalarType,Dimension> pmin{Infinity,Infinity},pmax{-Infinity,-Infinity};
	for(const CGT::Full_point & p : pts){
		for(int i=0; i<Dimension; ++i){
			pmin[i] = std::min(pmin[i],p.first.point()[i]);
			pmax[i] = std::max(pmax[i],p.first.point()[i]);
		}
	}
	std::array<ScalarType,Dimension> pdiff{pmax[0]-pmin[0],pmax[1]-pmin[1]};
	using WP = CGT::Weighted_point; using IT = CGT::InfoType;
	pts.push_back({WP({pmax[0]+pdiff[0],pmax[1]+pdiff[1]},0),IT(pts.size(),ExcludedBoundary)});
	pts.push_back({WP({pmin[0]-pdiff[0],pmax[1]+pdiff[1]},0),IT(pts.size(),ExcludedBoundary)});
	pts.push_back({WP({pmin[0]-pdiff[0],pmin[1]-pdiff[1]},0),IT(pts.size(),ExcludedBoundary)});
	pts.push_back({WP({pmax[0]+pdiff[0],pmin[1]-pdiff[1]},0),IT(pts.size(),ExcludedBoundary)});

	// Generate an initial guess, as a parabola, and check the convexity constraint
	std::vector<ScalarType> x;
	for(const CGT::Full_point & p : pts) x.push_back(0.5*CGT::Parabola(p.first));
	// Generate the principal agent objective function
	PrincipalAgent pa;
	{
		ConvexityConstraint cvx(pts);
		cvx.SetValues(x);
	...
	for(int i=0; i<pts.size()-4; ++i){pts[i].second.boundary = 0;}
	ConvexityConstraint cvx(pts);
	cvx.SetValues(x);

	
	BoundConstraint bd;
	bd.lb.resize(pts.size(),0.);
	bd.ub.resize(pts.size(),20.);
	bd.Init();
	std::vector<NS::Functionnal*> constraints = {&cvx, &bd};

		
	NS::NewtonConstrained newton;
	newton.maxIter=50;
	newton.opt.sDelta.stopBelow = 0.001;
	newton.verbose = true;
	//		newton.verbose=true;
