...
#ifndef CGalTest_DGO_Test_h
#define CGalTest_DGO_Test_h
        ...
        else
        {
            if ( yo )
            {
                if ( hessubi && hessubj )
                {
                    ...
                    std::cout << " ------- A priori hessian pattern ------------- \n";
 ...
 */
{
    ...
    if ( maxnumhesnz )
    {
        ...
        else
        {
            if ( numhesnz )
            {
                std::cout << " ------- A posteriori hessian pattern ------------- \n";
             ...
             nlhand_t  *nlh)
{
    ...
    if ( r == MSK_RES_OK )
    {
        ...
        if ( r==MSK_RES_OK )
        {
            ...
            if (r == MSK_RES_OK)
            {
                nlh[0]->p = static_cast<int*>(MSK_calloctask(task,nlh[0]->t+1,sizeof(int)));
                if (nlh[0]->p == NULL)
                    r = MSK_RES_ERR_SPACE;
            }
